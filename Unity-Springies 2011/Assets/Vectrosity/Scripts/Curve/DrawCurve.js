...
function SetLine () {
	if (useDottedLine) {
		line = new VectorLine("Curve", linePoints, dottedLineMaterial, 8.0, 0.0, 1, LineType.Continuous, Joins.Open);
	}
	else {
		line = new VectorLine("Curve", linePoints, lineMaterial, 4.0, 0.0, 1, LineType.Continuous, Joins.Open);		
	}
}
...
function AddPoint () {
	...
	Vector.DestroyLine(controlLine);
	controlLine = new VectorLine("Control Line", curvePoints, Color(0.0, .75, .1, .6), lineMaterial, 2.0);
	Vector.DrawLine(controlLine);
