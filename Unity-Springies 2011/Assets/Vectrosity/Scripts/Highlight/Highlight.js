...
function Start () {
	...
	line = new VectorLine("Line", linePoints, lineMaterial, lineWidth, lineWidth*.5);
	energyLinePoints = new Vector2[pointsInEnergyLine];
	energyLine = new VectorLine("Energy", energyLinePoints, lineMaterial2, energyLineWidth, 0, 0, LineType.Continuous, Joins.Open);
	SetEnergyLinePoints();
}

function SetEnergyLinePoints () {
	for (i = 0; i < energyLinePoints.Length; i++) {
